...
public readonly ref struct MapFeatureData
{
    ...
    //In order to optimize the rendering, we use a dictionary that uses int values
    public enum Types
    {
        Highway = 0,
        Water = 1,
    ...
    public Dictionary<Types, string> Properties { get; init; }
}
    ...
    public void ForeachFeature(BoundingBox b, MapFeatureDelegate? action)
    {
        ...
        for (var i = 0; i < tiles.Length; ++i)
        {
            ...
            for (var j = 0; j < header.Tile.Value.FeaturesCount; ++j)
            {
                ...
                if (isFeatureInBBox)
                {
                    var properties = new Dictionary<MapFeatureData.Types, string>(feature->PropertyCount);
                    for (var p = 0; p < feature->PropertyCount; ++p)
                    {
                        ...
                        //Store the properties used for computation in memory in order to speed up the process
                        if (LandTypes.Contains(key.ToString()))
                        {
                            try
                            {
                                ...
                                MapFeatureData.Types keyEnum =
                                    (MapFeatureData.Types)Enum.Parse(
                                        typeof(MapFeatureData.Types),
                                        ConvertString(key.ToString()));
